  ...
  if (enabledPlatforms !== undefined) {
    lines.push(
      enabledPlatforms.length === 0
        ? "当前没有启用发布平台。不要调用 video-publisher 或 jacky_creator_sync_publish。"
        : `当前启用平台：${enabledPlatformNames(enabledPlatforms)}。video-publisher 和 jacky_creator_sync_publish 只处理这些平台。`,
    );
  }
