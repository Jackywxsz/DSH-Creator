...
剪辑仍走 `screen-studio-editor`。上传优先使用已安装的 `jacky-video-publisher`；没有 Jacky 版时兼容公开的 `video-publisher`，不会把兼容包改名伪装成 Jacky Skill。这两项没有做成 Harness 工具。公众号图文走 `oil-video-article`。
...
设置在 Harness 的设置 → 插件 → Jacky Creator。这里会逐项显示 DSH 实际可发现的能力，并配置影片目录、`enabledPlatforms`、脚本规则（人设）和凭据。影片目录默认是 `~/Movies/视频项目`。小红书、抖音、B 站、视频号默认都不勾选；只勾选你确实拥有账号的平台。勾选只代表允许 AI 发布和数据同步，不代表已经登录。点“检测登录状态”会用 Ego Browser 只读访问创作者后台；需要登录或无法确认时，点“打开登录页”，登录完成后重新检测。全部关闭时不执行发布或同步。
...
「已发布，同步数据」从创作者后台回收播放、赞和评论。同步只翻启用平台的已发布列表，再对到本地片子；90 秒内再点一次会直接用刚才的结果。每次同步开一个新的 Ego 空间，采完就关掉。平台上有、本地没有文件夹的，不会自动新建一条片子。

运营页的六维评分现在是展开策略后才显示的可选发布前诊断，不是发布门槛。评分建议只有点击「采纳首条为下一步」才会写入下一步。复盘页默认可直接人工填写「本次结果、有效做法、存在问题、下一次实验」并保存草稿，不需要 AI 会话；AI 只负责可选起草。无论草稿来自哪里，都必须人工确认后才写入 Markdown，规则和模板还要再确认一次。
...
同步成功后，播放、赞、评论和链接写入 Jacky Creator 的本地状态文件。检查器的视频平台卡会显示这些数字，并提供「打开作品」。同步会翻完各平台创作者后台的已发布列表；平台上有、本地没有文件夹的不会自动建条目。视频号需要 Ego 里已经登录创作者后台。
